...
def move_until_marker(next_id): # needs to change
	global currentLoc,reached, up
	drive_pub.publish("up")
	wait=False
	while wait == False:
		marker=int(req_front().split(',')[1])

		if up==1:
			if int(marker)>int(next_id) and int(marker)<10:
				drive_pub.publish("rup")
				time.sleep(1)
				direction("comp")
				up=0
...
def move_funct_front(current_id,next_id):
	...
	#direction(direc)
	pub.publish("")
	pub.publish("reached:"+str(next_id))
...
def callback_goal(data):
	...
	if sett == True :
		...
		tt = int(data.data)
		if tt >= 0 :
			move_funct_front(currentLoc,tt)
		# elif tt==0 :
		# 	move_funct_front(99,5)
...
# def move(position):
# 	global currentLoc,reached,sett,previous
# 	sett=True
