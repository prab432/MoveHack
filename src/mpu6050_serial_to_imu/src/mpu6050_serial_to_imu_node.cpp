...
int main(int argc, char** argv)
{
  ...
  private_node_handle.param<std::string>("port", port, "/dev/serial/by-path/platform-xhci-hcd.3.auto-usb-0:1.2:1.0-port0");
  ...
  ros::Publisher imu_pub = nh.advertise<sensor_msgs::Imu>("data", 3);
  ros::Publisher imu_temperature_pub = nh.advertise<sensor_msgs::Temperature>("temperature", 3);
          ...
          while (input.length() >= 28) // while there might be a complete package in input
          {
            ...
            if (data_packet_start != std::string::npos)
            {
              ...
              if ((input.length() >= data_packet_start + 28) && (input.compare(data_packet_start + 26, 2, "\r\n") == 0))  //check if positions 26,27 exist, then test values
              {
                ...
                //differential_rotation.getRPY(roll, pitch, yaw);
                //ROS_DEBUG(yaw);
